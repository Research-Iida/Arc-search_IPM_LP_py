...
def test_reverse_non_lower_bound():
    ...
    A = np.array([[1, 2], [3, 4]])
    c = np.array([1, 1])
    test_lb, test_ub, test_A, test_c = LP.reverse_non_lower_bound(
        lb, ub, A, c
    )
...
def test_separate_free_variable():
    ...
    A = np.array([[1, 2], [3, 4]])
    c = np.array([1, 1])
    test_lb, test_ub, test_A, test_c = LP.separate_free_variable(
        lb, ub, A, c
    )
    np.testing.assert_array_equal(test_lb, [0, 1, 0])
    np.testing.assert_array_equal(test_ub, [np.inf, 2, np.inf])
    np.testing.assert_array_equal(test_A, [[1, 2, -1], [3, 4, -3]])
...
def test_make_A():
    ...
    np.testing.assert_almost_equal(A, sol_A)
...
def test_make_b():
    ...
    b = LP.make_standard_b(
        A_E, A_G, A_L, b_E, b_G, b_L, lb, ub
    )
    sol_b = np.array([
        1, 1,
        2,
...
def test_convert_standard():
    ...
    np.testing.assert_array_equal(test_LP.A, [[0, 1, -1, 0], [-2, 3, -3, -1]])
    np.testing.assert_array_equal(test_LP.b, [2, 4 - (-2 * -5)])
    np.testing.assert_array_equal(test_LP.c, [-6, 7, -7, 0])


# def test_create_A_row_normalized():
#     A = np.array([[1, 2, 6], [3, 4, 0]])
#     b = np.array([5, 10])
#     aLPS = LPS(A, b, np.zeros(3))

#     test_LPS = aLPS.create_A_row_normalized()
#     for i in range(aLPS.m):
#         np.testing.assert_almost_equal(test_LPS.A[i, :], A[i, :] / np.linalg.norm(A[i, :]))
#         assert np.isclose(np.linalg.norm(test_LPS.A[i, :]), 1)
#         np.testing.assert_almost_equal(test_LPS.b[i], b[i] / np.linalg.norm(A[i, :]))
